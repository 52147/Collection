...
public abstract class Triangle extends ShapeBase implements TriangleInterface{
	...
	private void drawTop() {
		...
		// insideWidth == number of spaces between the 2 '*'s on a line.
		int insideWidth = 1;
		
		for(int count = 0; count < lineCount; count++) {
			// Down one line, so the first '*' is one more space to left.
			startOfLine --;
			skipSpaces(startOfLine);
			System.out.println('*');
			...
			// Down one line, so the inside is 2 space wider.
